...
function transformToLis(obj){
  // Solution code here...
  let newArr = [];
  Object.entries(obj).forEach(field => newArr.push(`<li>${field[0]}: ${field[1]}</li>`));
  return newArr;
}
...
const count = (target, input) => {
  // Solution code here...
  let counter = 0;
  input.map(array => array.forEach(int => target === int ? counter++ : counter));
  return counter;
};
...
const totalSum = (input) => {
  // Solution code here...
  let sum = 0;
  input.map(array => sum += array.reduce((acc,val) => acc += val));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArr = [];
  input.map(array => newArr.push(array.filter(int => Number.isInteger(int) && int % 5 === 0)));
  newArr.map(array => array.forEach((int, idx, array) => array[idx] = Math.pow(2, int)));
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newArr = [];
  data.map(character => character.gender === 'male' || character.gender === 'female' ? newArr.push(character.name): '');
...
let findShortest = (data) => {
  // Solution code here...
  let newArr = data.sort((a,b) => a.height - b.height);
